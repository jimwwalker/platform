...
PLATFORM_PUBLIC_API void JEArenaMalloc::switchToClient(
    ...
    if (client.threadCache && tcacheEnabled) {
        ...
        // The following (and else if) are required to safely ensure the folly
        // ThreadLocal is initialised against the default arena, this avoids
        // recursive death.
        tld.client = ArenaMallocClient{0, NoClientIndex, tcacheEnabled};
