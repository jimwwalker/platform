...
// present, e.g. some of the sanitiser builds. However for easier testing it is
// permissible to run use je_malloc or std::malloc (and friends) as the
// underlying allocator. For example on OSX where no malloc_usable_size is
// available from the 'system' we would struggle to test memory tracking, but if
// we choose jemalloc we can. Todo this actually requires a manual modification
// to 1) build this file when HAVE_JEMALLOC is true, and 2) force the
// cb_arena_alloc_client to pick the SystemArenaMalloc client as the
// implementation.
...
void SystemArenaMalloc::registerTotalCounter(
        const ArenaMallocClient& client,
        cb::RelaxedAtomic<int64_t>* counterAddress) {
    clients.wlock()->at(client.index).clientTotal = counterAddress;
}
...
void SystemArenaMalloc::unregisterCurrentClient() {
    unregisterClient(currentClient);
}
...
size_t SystemArenaMalloc::getAllocated(const ArenaMallocClient& client) {
    // Just read from client's index into the allocations array
    return allocated[client.index];
}

void SystemArenaMalloc::updateTotalCounters() {
    auto lockedClients = clients.rlock();
    for (uint8_t index = 0; index < lockedClients->size(); index++) {
        const auto& client = lockedClients->at(index);
        if (client.used && client.clientTotal) {
            client.clientTotal->store(allocated[index].load());
        }
    }
}
