...
struct PLATFORM_PUBLIC_API ArenaMallocAutoSwitchFrom {
    // Destructor will call ArenaMalloc::switchFromClient()
    ~ArenaMallocAutoSwitchFrom();
};
...
public:
    ...
    /**
     * Register a counter that will be periodically updated with the client's
     * allocation size.
     * @param client The client's handle
     * @param counterAddress pointer to a relaxed-atomic which will be written
     *  to by a periodic update (the MemoryTracker thread)
     */
    static void registerTotalCounter(
            const ArenaMallocClient& client,
            cb::RelaxedAtomic<int64_t>* counterAddress) {
        return Impl::registerTotalCounter(client, counterAddress);
    }
    ...
    /**
     * Unregister the current client, i.e. the last switchToClient on the
     * current thread.
     */
    static void unregisterCurrentClient() {
        Impl::unregisterCurrentClient();
    }
    ...
    /**
     * Switch to the given client, all subsequent memory allocations made by
     * the current thread will be accounted to the client. This variant returns
     * an object that when destructed will automatically call switchFromClient
     * so subsequent allocation activity occurs against the default arena.
     * @param client The client to account to.
     * @return an object which will call switchFromClient on destruction
     */
    static ArenaMallocAutoSwitchFrom switchToClientAuto(
            const ArenaMallocClient& client) {
        Impl::switchToClient(client);
        return ArenaMallocAutoSwitchFrom{};
    }
    /**
    ...
    static size_t getAllocated(const ArenaMallocClient& client) {
        return Impl::getAllocated(client);
    }

    /**
     * Update all counters which have been registered via registerTotalCounter
     */
    static void updateTotalCounters() {
        return Impl::updateTotalCounters();
    }
